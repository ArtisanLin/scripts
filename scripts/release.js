...
let skipTests = true || args.skipTests
...
const packages = fs
  ...
  .map(p => {
    ...
    if (!fs.statSync(pkgRoot).isDirectory()) return null
    ...
    return {
      name: p,
      path: pkgRoot,
      order: pkg.order ?? 0, // 没写 order 的默认是 0
      pkgJson: pkg,
    }
  })
...
const dryRun = async (
  ...
  /** @type {ReadonlyArray<string>} */ args,
  /** @type {import('execa').Options} */ opts = {},
) => console.log(picocolors.blue(`[dryrun] ${bin} ${args.join(' ')}`), opts)
const runIfNotDry = isDryRun ? dryRun : run
const getPkgRoot = (/** @type {string} */ pkg) =>
  path.resolve(__dirname, '../packages/' + pkg)
const step = (/** @type {string} */ msg) => console.log(picocolors.cyan(msg))
...
async function main() {
  ...
  for (const pkg of packages) {
    // TODO: 修改命令 npm 地址
    await publishPackage(pkg, targetVersion, ['--registry=http://nexus.custom.cn/repository/npm-host/', ...additionalPublishFlags])
    // await publishPackage(pkg, targetVersion, ['--registry=https://registry.npmjs.org/', ...additionalPublishFlags])
  }
...
function updateDeps(pkg, depType, version, getNewPackageName) {
  ...
  Object.keys(deps).forEach(dep => {
    if (isCorePackage(dep)) {
      ...
      console.log(
        picocolors.yellow(`${pkg.name} -> ${depType} -> ${dep}@${newVersion}`),
      )
      deps[dep] = newVersion
    }
  })
