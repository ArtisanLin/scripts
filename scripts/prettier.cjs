...
function runPrettier(options) {
  const { changedFiles, shouldWrite, debug } = options
  let didWarn = false
  let didError = false
  const warnedFiles = []
  const ignoredFiles = fs__default
    .readFileSync(path__default.join(workspaceRoot, 'scripts', '.prettierignore'), 'utf-8')
    .split(/\r*\n/)
    .filter((line) => {
      return (
        // ignore comments
  ...
  files.forEach(async (file) => {
    ...
    try {
      ...
      if (shouldWrite) {
        ...
        if (output !== input) {
          if (debug) console.log(`Formatting ${file}`)
...
async function run(argv) {
  ...
  runPrettier({ changedFiles, shouldWrite, branch, debug })
}
commander.program
